    ...
    <article className="min-h-dvh dr-layout-grid-inner dr-py-32">
      <div className="col-span-full dt:col-start-4 dt:col-end-10">
        ...
        <nav className="dr-mb-40">
          <Link
            href="/blog"
            className="inline-flex items-center gap-2 typo-p dr-text-14 text-off-white no-underline hover:text-forest transition-colors duration-150"
          >
        ...
        {(author || date) && (
          <div className="dr-mb-48 text-center">
            <div className="inline-flex items-center dr-gap-16 dr-px-24 dr-py-12 bg-white rounded-full typo-p dr-text-14 border border-dark-grey">
              {author && (
                <span className="font-medium text-off-white">By {author}</span>
              )}
              {author && date && <span className="text-off-white">•</span>}
              {date && (
                <span className="text-off-white font-medium">{date}</span>
              )}
            </div>
          </div>
        )}
